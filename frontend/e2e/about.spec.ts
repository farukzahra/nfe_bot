...
test.describe('About', () => {
  test('shows commit history table after login', async ({ page }) => {
    ...
    await expect(page.getByTestId('commit-history-table')).toBeVisible({ timeout: 15000 })
    ...
    await page.waitForLoadState('networkidle')
    await expect(page.getByTestId('commit-history-table')).toBeVisible({ timeout: 10000 })
  })
})
